...
function getCashRegisterMovements($cashRegisterSessionId) {
    global $pdo;
    $stmt = $pdo->prepare("
        SELECT id, 'sale' as movement_type, total_amount as amount, sale_date as created_at, CONCAT('Venta #', id) as notes
        FROM sales
        WHERE cash_register_session_id = :cash_register_session_id
        UNION ALL
        SELECT id, 'purchase' as movement_type, total_amount as amount, purchase_date as created_at, CONCAT('Compra #', id) as notes
        FROM purchases
        WHERE cash_register_session_id = :cash_register_session_id
        UNION ALL
...
function getMovementById($id) {
    global $pdo;
    $stmt = $pdo->prepare("SELECT * FROM cash_register_movements WHERE id = ?");
    $stmt->execute([$id]);
    return $stmt->fetch(PDO::FETCH_ASSOC);
}

function updateCashRegisterMovement($id, $movementType, $amount, $notes) {
    global $pdo;
    try {
        $stmt = $pdo->prepare("UPDATE cash_register_movements 
                               SET movement_type = ?, amount = ?, notes = ? 
                               WHERE id = ?");
        $stmt->execute([$movementType, $amount, $notes, $id]);
