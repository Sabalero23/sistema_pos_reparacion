...
if (!isLoggedIn() || !hasPermission('cash_register_view')) {
    setFlashMessage("No tienes permiso para ver este comprobante.", 'error');
    redirect('index.php');
}
...
if (!$sessionId) {
    setFlashMessage("ID de sesión de caja no proporcionado", 'error');
    redirect('cash_register.php');
}

$session = getCashRegisterSummary($sessionId);

if (!$session) {
    setFlashMessage("Sesión de caja no encontrada", 'error');
    redirect('cash_register.php');
}

$movements = getCashRegisterMovements($sessionId);
$purchases = getPurchasesForSession($sessionId);
...
$expectedBalance = $session['opening_balance'] + $totals['sale'] + $totals['cash_in'] - $totals['purchase'] - $totals['cash_out'];
$difference = $session['closing_balance'] - $expectedBalance;

$pageTitle = "Comprobante de Cierre de Caja #" . $session['id'];
